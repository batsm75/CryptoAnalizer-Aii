...
def ask_ai_mentor_gemini(user_question):
    ...
    if not GEMINI_API_KEY: 
        print_wrapped("⚠ Error: GEMINI_API_KEY belum diatur. Harap dapatkan kunci API Gemini Anda dari Google AI Studio. Fitur ini tidak dapat digunakan.", width=get_terminal_width(), initial_indent="")
        return "ERROR: API Key tidak ada." # Mengembalikan pesan error untuk ditampilkan
    ...
    if is_crypto and candlestick_patterns: # Hanya tampilkan di crypto
        ...
        if candlestick_patterns and candlestick_patterns[0]["name"] == "DATA TIDAK CUKUP":
            ...
            for i, pattern in enumerate(candlestick_patterns):
                ...
                indent = "     " # Indent for description and action
                max_line_width = width - len(indent) - 2 # Kurangi padding border dan bullet/nomor
                
                print_wrapped(f"  {i+1}. Pola: {pattern['name']} ({pattern['type']})", width=width, initial_indent="  ", subsequent_indent="     ")
...
def handle_ai_questions(coach_suggestion, market_psychology, cut_loss_plan, asset_name, is_crypto=True, is_forex_simulated=False):
    ...
    if choice == '1':
        ...
        print_wrapped("\nAI Agent: Maaf, saya tidak memahami pertanyaan Anda. Sil
